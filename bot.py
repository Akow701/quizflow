...
def quiz_menu(quiz):
    ...
    kb = [
        ...
        [InlineKeyboardButton("👥 Start in group", switch_inline_query=f"quiz:{quiz['quiz_id']}")],
        [InlineKeyboardButton("📤 Share quiz", callback_data=f"share:{quiz['quiz_id']}")],
        [InlineKeyboardButton("✏️ Edit title", callback_data=f"edittitle:{quiz['quiz_id']}"),
         InlineKeyboardButton("🗑 Delete", callback_data=f"delete:{quiz['quiz_id']}")],
        [InlineKeyboardButton("📊 Stats", callback_data=f"stats:{quiz['quiz_id']}")],
    ]
    text = (
        f"📋 *{quiz['title']}*\n"
        f"❓ {count} ta savol\n"
        f"⏱ 30 soniya har savol\n\n"
        f"🔗 `t.me/{(os.environ.get('BOT_USERNAME','QuizBot'))}?start=quiz_{quiz['quiz_id']}`"
    )
