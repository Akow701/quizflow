...
async def handle_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if data.startswith("view:"):
            ...
            quiz = get_quiz(quiz_id)
            if not quiz:
                await q.answer("❌ Quiz topilmadi.", show_alert=True)
                return
            bot_username = (await ctx.bot.get_me()).username
            text, kb = quiz_menu(quiz, bot_username)
            ...
            await q.answer()
            import asyncio
            asyncio.ensure_future(send_next_question(chat_id, session_id, ctx.application))
        ...
        try:
            await q.answer("❌ Xato yuz berdi, qayta urinib ko'ring.", show_alert=True)
        except Exception:
            pass
